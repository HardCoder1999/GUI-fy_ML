...
class prepare_data:
	...
	def split_data(self, dataFrame, test_size, label_column, shuffle, filepath):
		X, y = self.return_feature_label(dataFrame, label_column)
		X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=2, shuffle=shuffle)
...
class knn_model(prepare_data):
	...
	def train_model(self, X_train, y_train, modelpath):
		model = KNeighborsClassifier(n_neighbors=self.n_neighbors, weights=self.weights, 
			algorithm=self.algorithm, p=self.power_param)
...
class radius_nn_model(prepare_data):
	...
	def train_model(self, X_train, y_train, modelpath):
		model = RadiusNeighborsClassifier(radius=self.radius, weights=self.weights, 
			algorithm=self.algorithm, p=self.power_param, outlier_label=self.outlier_label)
	...
	def save_model(self, model, modelpath):
		joblib.dump(model, os.path.join(modelpath, 'radius_nn_model.pkl'))
...
class svm_model(prepare_data):
	...
	def save_model(self, model, modelpath):
		joblib.dump(model, os.path.join(modelpath, 'svm_model.pkl'))
...
class linear_svc_model(prepare_data):
	...
	def __init__(self, penalty='l2', loss='squared_hinge', dual=True, C=1.0, multi_class='ovr'):
	...
	def save_model(self, model, modelpath):
		joblib.dump(model, os.path.join(modelpath, 'linsvc_model.pkl'))
...
class logreg_model(prepare_data):
	...
	def save_model(self, model, modelpath):
		joblib.dump(model, os.path.join(modelpath, 'logreg_model.pkl'))
